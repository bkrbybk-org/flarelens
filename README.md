...
Ops dashboard for Cloudflare: one pane of glass for reviewing an account's **Zero Trust** configuration and telemetry, **security** posture (WAF, AI Security, PQC readiness, request forensics, client & API protection), **cache rules**, and **developer-platform usage** — nineteen sections served entirely from one Cloudflare Worker.
...
| `#/shields` | Page & API Shield | account, optional zone (`?sh_zone=<id>`) | **Page Shield** (verified live): per-zone enabled/disabled, scripts and connections observed on the page (host, first/last seen, page count, status, malicious-domain/URL flags, third-party detection against the zone's own domain), and content-security policies. Rendered as text, never as auto-navigating links — these hosts and URLs are third-party-controlled strings. **API Shield** (built from Cloudflare's documented API shapes, **not verified against a live account** — every `/api_gateway/*` read this app tried returned "Authentication error" with the bound token): saved vs. discovered-but-unsaved endpoints, schema validation mode and uploaded schemas, session identifier configuration — each zone shows "not checked — missing permission" rather than a bare zero when the read fails. **Findings:** Page Shield disabled (low), malicious domain/URL flag (high), new third-party script within 7 days (info), zero policies with scripts present (low), discovered endpoints not saved (low), schema validation absent/log-only with schemas uploaded (medium), no session identifier (info) — API Shield findings only ever come from a read that actually succeeded. Scripts/connections CSV export, capped at 500 rows per zone with a truncation flag |
...
| `#/findings` | Findings | account (+ loaded sections) | Severity-ranked audit view across every section. **Access/Groups:** publicly-reachable apps, apps with no policy, `bypass` decisions, unreferenced groups, sessions over 24h (medium from 7 days), session cookies without `HttpOnly`, CORS wildcards (medium with credentials), and allow policies admitted by email domain, login method or service token alone with no `require`. **WAF:** action drift, plus an evaluation-order pass — rules that never run, one finding per disabled deployment or upstream stopper rather than one per rule, and undeployed rulesets. **Cache:** insights and health grade. **Tunnels:** ungated hostnames (high), destinations with no route (medium), and health notes (medium for a warning, low for an info note such as version skew). **Zone Health:** its own certificate-expiry and DNS-hygiene findings, at their own severities — a check it could not run is never folded in as clean. **PQC:** hostnames not ready for post-quantum key exchange (medium) and zone-level TLS hygiene gaps (low). **DNS Records:** an exposed origin address (medium) or a published internal address (low) on an unproxied record. **Rate Limits & Bots**, **Gateway Policies** and **Page & API Shield:** their own findings as reported, `info` shown as low. Every source states, per source, whether it was checked (with a count), is still loading, could not be checked (with the reason — a failed fetch or a missing permission), or has not been opened yet (WAF, Cache). A source the token cannot read never ends the session: only a 401 does (see `isSessionError` in `web/src/api/client.ts`). Filterable by source; a **Report** action exports a self-contained, client-facing HTML posture summary — coverage, severity totals, a table per section, and a "what was not checked" list — safe to print to PDF, with no scripts or external resources |

Sections carry deep-linkable state, e.g. `#/waf?zone=<id>&lookback=1440&tab=rules` or `#/cache?zone=<id>&range=168`.

Findings always covers Access and Groups. Tunnels, Zone Health, PQC, DNS Records and Rate Limits & Bots are fetched by the Findings page itself, concurrently, through the same edge-cached routes their own pages use — cheap, since a hit there is a 60s server-side cache hit, not a fresh Cloudflare fan-out. WAF and Cache are zone-scoped and fetched by their own pages instead, so their findings fold in only once you have opened those sections — the page says so explicitly per source rather than implying a clean bill of health it has not checked.

**Export:** the Access table and the Findings page export CSV (respecting the active filters and visible columns; UTF-8 with a byte-order mark, and any cell that would run as a spreadsheet formula starts with an apostrophe so it stays text). Findings also has a print stylesheet for Save-as-PDF, and a **Report** button that downloads a self-contained HTML file — inline CSS, no scripts, no external resources — built by [web/src/lib/report.ts](web/src/lib/report.ts)'s `buildReportHtml`, with every interpolated string passed through `escapeHtml` first since finding text originates upstream and must never become markup.
...
src/lib/cloudflared-version.ts  Numeric YYYY.M.P version compare against the latest cloudflared
                             GitHub release (1h edge-cached); the Worker's first call to a
                             non-Cloudflare service — see below
src/lib/tunnel-metrics.ts    Connector CPU/memory/HA metrics: TUNNEL_METRICS config parsing and
                             validation, a strict Prometheus text-format parser, and the secured
                             fetch (Access service token, no redirects, capped body, timeout)
...
src/lib/shields.ts           Page & API Shield: Page Shield scripts/connections/policies (verified
                             live), API Shield endpoints/schema validation/session id (documented
                             shapes, unverified live — every /api_gateway/* read returned an auth error)
...
web/src/lib/findings.ts      Findings: Access/Groups/WAF/Cache mappers, sorting, severity counts
web/src/lib/findings-sources.ts
                             Findings: Tunnels/Zone Health/PQC/DNS Records/Bots mappers, WAF evaluation-order findings
web/src/lib/report.ts        Findings' "Report" export: buildReportHtml (pure) + escapeHtml + downloadHtml
...
| Client-side Security: Read | Page Shield status, scripts, connections and policies in Page & API Shield. Named "Page Shield: Read" in older Cloudflare docs; Cloudflare answers a token without it with a 401/403 or code 10000, and each read is reported as not checked, naming this scope — never as a zone with nothing observed |
| API Gateway: Read | Saved/discovered endpoints, schema validation settings, uploaded schemas and session identifier configuration in Page & API Shield's API Shield tab. **Unverified against a live account** — every `/api_gateway/*` read this app tried returned "Authentication error" with the token available while building it, so this scope's effect is inferred from Cloudflare's published API resources, not confirmed live |
...
### Comparing against the latest cloudflared release

The Tunnel Map's connector version comparison is this app's **first call to a non-Cloudflare
service**: the Worker fetches `https://api.github.com/repos/cloudflare/cloudflared/releases/latest`
with a `User-Agent: flarelens` header (GitHub requires one) and nothing else — no account data, no
credentials, no query parameters. The response (`tag_name`, `published_at`) is cached at the edge
for 1 hour under a fixed key, since it is public data with no per-caller scope
(`getLatestCloudflaredCached` in [src/lib/cloudflared-version.ts](src/lib/cloudflared-version.ts),
alongside but separate from the per-credential cache in
[src/lib/edge-cache.ts](src/lib/edge-cache.ts)). A failure — rate limiting, a network error, an
unparsable tag — degrades to `{ error: "…" }` and is shown as "latest version unknown, <reason>";
it never fails the tunnel map itself. Versions are compared numerically (`year`, `month`, `patch`),
not as strings or semver — cloudflared's `YYYY.M.P` scheme would otherwise sort `2026.9.1` after
`2026.10.1`. A connector more than ~6 months behind the latest release gets a `warn`-level health
note; anything older but within that window gets `info`.

### Connector metrics

Cloudflare's API does not report a `cloudflared` connector's CPU or memory — `cloudflared` only
exposes those (plus HA connection count, request/error totals and concurrent requests) on its own
Prometheus `/metrics` endpoint, on the connector's own host. To let Flarelens read it, publish that
endpoint through a Cloudflare Tunnel of its own, protected by Access:

1. On the connector host, run `cloudflared` with `--metrics 127.0.0.1:20241` (its default) or
   point `--metrics` at a different local address.
2. Add a **public hostname** to a tunnel (the same one or a different one) whose service is that
   local metrics address, e.g. `https://metrics-connector-a.example.com` → `http://127.0.0.1:20241`.
3. Put a Cloudflare **Access application** in front of that hostname, with a policy that admits
   only a **service token** (Access → Service Auth) — never a login policy, since nothing here can
   complete a browser login.
4. Configure the deployment with three secrets and one JSON blob of targets:

```bash
wrangler secret put METRICS_ACCESS_CLIENT_ID       # the service token's Client ID
wrangler secret put METRICS_ACCESS_CLIENT_SECRET   # the service token's Client Secret
wrangler secret put TUNNEL_METRICS
# when prompted, paste JSON like:
# [{"tunnelId":"<cfd_tunnel id>","url":"https://metrics-connector-a.example.com/metrics"}]
```

**Security properties, all enforced server-side, none client-influenceable:**

- Targets exist only in this deploy-time secret. A request names a tunnel id; the Worker looks up
  the URL — a client can never supply or discover a URL, host, or path. The map result exposes only
  a `hasMetricsTarget: boolean` per tunnel, never the configured URL.
- Each target is validated at use, not trusted from the secret: `https:` only, no userinfo
  (`user:pass@`) in the URL, hostname must not be `localhost` or an IP literal, and the path must
  end in `/metrics`. An invalid entry is reported as misconfigured rather than fetched.
- The requested tunnel id must belong to the caller's allowlisted/authenticated account, checked
  against Cloudflare directly before any metrics fetch.
- The fetch uses a 5s timeout, follows no redirects (`redirect: "manual"` — a redirect almost
  always means Access is rejecting the service token and returning its login page, which this app
  must never treat as a 200), and caps the response body at 2 MB before parsing.
- Only the parsed numbers are returned to the browser — never the upstream response body or
  headers, and never the configured URL.
- The route (`GET /api/tunnels/:tunnelId/metrics`) is not edge-cached; it is an on-demand "Load
  metrics" action in the tunnel drawer, not part of the polled/cached map.

CPU is reported as a **cumulative counter** (`process_cpu_seconds_total`); a true instantaneous
utilization needs two samples a few seconds apart, which this app does not take. Instead it shows
**average CPU utilization since process start** — `process_cpu_seconds_total` divided by uptime
(`process_start_time_seconds`) — labelled exactly that, not as a live reading.
