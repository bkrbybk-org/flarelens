...
| `#/request` | Request Trace | account or zone | Everything Cloudflare records about one HTTP request, found by Ray ID: the request row, client fingerprints, security verdicts and edge timings, plus every firewall rule that matched. Field selection follows a live schema probe, and absence is reported as inconclusive because `httpRequestsAdaptive` is adaptively sampled |
