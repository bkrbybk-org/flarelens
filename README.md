...
|---|---|---|
| Unit | `tests/{expr,rules,csv,findings,cache-analysis,waf-meta,waf-aggregate,waf-chart,hash-params,auth,chart-hover,pqc,tunnel-sankey}.test.ts`, `tests/ai-sec-*.test.ts` | Pure logic: wirefilter evaluation, rule rendering, findings, CSV, WAF aggregation and bucketing, hash deep-link helpers, Access JWT verification, chart hover placement, PQC verdicts, the Tunnel Map flow diagram's geometry (every column sums to the row count, no band overflows its node), and the AI Security domain layer including `buildDashboard` |
