...
| `#/groups` | Access Groups | account | Two tabs over the account-level configuration applications attach by reference: **Rule groups** (Access Groups with their rules, cross-referenced to the applications whose policies use them) and **Reusable policies** (account-level policies with their decision, rules and the applications they are attached to — a policy attached to nothing is called out as enforcing nothing). Deep-linkable as `#/groups?tab=policies` |
