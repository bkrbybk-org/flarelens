...
| `#/groups` | Access Groups | account | Two tabs over the account-level configuration applications attach by reference. **Reusable policies** (default) is a sortable, filterable table — decision, rule counts, attached applications, created/updated — sorted by last update, with Excel-style column filters, search, CSV export and an expandable row showing the policy's rules; a policy attached to nothing is flagged, since it enforces nothing. **Rule groups** lists Access Groups with their rules, cross-referenced to the applications whose policies use them. Deep-linkable as `#/groups?tab=groups` |
