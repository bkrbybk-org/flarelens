...
import {fetchGatewayPoliciesReport, isSessionError } from "../../api/client";
import { useEstimatedProgress } from "../../hooks/useEstimatedProgress";
import type { GwReport } from "./types";

interface State {
	result: GwReport | null;
	loading: boolean;
	error: string | null;
...
export function useGatewayPoliciesReport(onAuthError: () => void) {
	const [state, setState] = useState<State>({ result: null, loading: false, error: null, cachedAt: null });
	const progress = useEstimatedProgress("cf_gateway_policies_last_load_ms");
	...
	const load = useCallback(
		async (token: string, accountId: string, fresh?: boolean) => {
			...
			let success = false;
			try {
				const { result, cachedAt } = await fetchGatewayPoliciesReport<GwReport>(token, accountId, { fresh });
				...
				setState((prev) => ({
					...prev,
					loading: false,
					error: err instanceof Error ? err.message : "Failed to build the Gateway Policies report",
				}));
			} finally {
